"""
AbeBooks scraper for fetching book pricing data via Decodo API.
...
from shared.abebooks_parser import parse_abebooks_html, extract_ml_features
from shared.timing import timed


# Base URL for AbeBooks search
ABEBOOKS_BASE_URL = "https://www.abebooks.com/servlet/SearchResults?isbn={isbn}&sortby=17"


@timed("AbeBooks fetch", log=False)
def fetch_abebooks_data(isbn: str, decodo_client: Optional[DecodoClient] = None) -> Dict[str, Any]:
    """
    Fetch and parse AbeBooks data for an ISBN using Decodo API.
    ...
    Examples:
        ...
        >>> data = fetch_abebooks_data("9780553381702", client)
    ...
    # Construct URL
    url = ABEBOOKS_BASE_URL.format(isbn=isbn_clean)
    ...
    if decodo_client is None:
        ...
        decodo_client = DecodoClient(
            username=username,
            password=password,
            plan="core"  # Use Core plan for AbeBooks
        )
        close_client = True
    ...
    try:
        ...
        # Parse HTML
        result = parse_abebooks_html(response.body, isbn_clean)
...
def _empty_result(error: str = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "ml_features": {
            "abebooks_min_price": None,
            "abebooks_avg_price": None,
            "abebooks_seller_count": 0,
...
def fetch_abebooks_bulk(isbns: list[str], progress_callback=None) -> Dict[str, Dict[str, Any]]:
    """
    Fetch AbeBooks data for multiple ISBNs using Decodo API.
    ...
    Returns:
        Dict mapping ISBN -> abebooks data dict

    Example:
        >>> results = fetch_abebooks_bulk(["9780553381702", "9780439708180"])
    ...
    try:
        ...
        for i, isbn in enumerate(isbns, 1):
            result = fetch_abebooks_data(isbn, client)
            results[isbn] = result
...
if __name__ == "__main__":
    ...
    print("Testing AbeBooks Scraper (via Decodo Core Plan)")
    print("-" * 80)
    ...
    data = fetch_abebooks_data(test_isbn)
