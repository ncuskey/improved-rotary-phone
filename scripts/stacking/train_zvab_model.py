#!/usr/bin/env python3
"""
Train Zvab specialist model for stacking ensemble.

Trains a GradientBoostingRegressor optimized for Zvab sold comps prediction
using Zvab-specific features (market signals, demand, condition).
"""
...
def extract_features(records, targets, extractor, catalog_db_path):
    """Extract Zvab-specific features from records."""
...
def train_zvab_model():
    """Train and save Zvab specialist model."""
    print("=" * 80)
    print("TRAINING ZVAB SPECIALIST MODEL")
    print("=" * 80)

    # Load data
    print("\n1. Loading Zvab training data...")
    platform_data = load_platform_training_data()
    zvab_records, zvab_targets = platform_data['zvab']

    print(f"\n   Loaded {len(zvab_records)} Zvab books")
    print(f"   Target range: ${min(zvab_targets):.2f} - ${max(zvab_targets):.2f}")
    print(f"   Target mean: ${np.mean(zvab_targets):.2f}")

    # Extract features
    print("\n2. Extracting Zvab-specific features...")
    extractor = PlatformFeatureExtractor()
    catalog_db_path = Path.home() / '.isbn_lot_optimizer' / 'catalog.db'
    X, y, isbns, completeness = extract_features(zvab_records, zvab_targets, extractor, catalog_db_path)

    feature_names = PlatformFeatureExtractor.get_platform_feature_names('zvab')
    ...
    # Save model
    model_path = model_dir / 'zvab_model.pkl'
    joblib.dump(model, model_path)
    print(f"✓ Saved model: {model_path}")

    # Save scaler
    scaler_path = model_dir / 'zvab_scaler.pkl'
    ...
    metadata_path = model_dir / 'zvab_metadata.json'
    with open(metadata_path, 'w') as f:
        json.dump(metadata, f, indent=2)
    print(f"✓ Saved metadata: {metadata_path}")

    print("\n" + "=" * 80)
    print("ZVAB MODEL TRAINING COMPLETE")
    print("=" * 80)
