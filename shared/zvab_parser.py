"""
Zvab HTML parser for extracting book pricing and availability data.
Zvab (Zentrales Verzeichnis Antiquarischer Bücher) is the German-language AbeBooks.
"""
...
def _extract_price(price_text: str) -> Optional[float]:
    """Extract numeric price from text (handles € and $ currencies)."""
    if not price_text:
        return None
    # Remove currency symbols, whitespace, and €/$/EUR
...
def _extract_condition(text: str) -> Optional[str]:
    """Extract and standardize book condition."""
    if not text:
        return None
...
def _extract_binding(text: str) -> Optional[str]:
    ...
    bindings = {
        "hardcover": "Hardcover",
        "gebunden": "Hardcover",  # German
        "softcover": "Softcover",
        "paperback": "Paperback",
        "taschenbuch": "Paperback",  # German
...
def parse_zvab_html(html: str, isbn: str) -> Dict[str, Any]:
    """Parse Zvab search results HTML and extract book pricing data."""
    soup = BeautifulSoup(html, 'html.parser')

    offers = []

    # Find result items (similar structure to AbeBooks)
    ...
    if not result_items:
        # Handle € and $ prices
        price_elements = soup.find_all(text=re.compile(r'EUR\s*\d+|€\s*\d+|US\$\s*\d+|\$\s*\d+'))
    ...
    for item in result_items:
        try:
            # Extract price
            price_text = ""
            price_elem = item.find(string=re.compile(r'EUR\s*\d+|€\s*\d+|US\$\s*\d+|\$\s*\d+'))
            if not price_elem:
                price_elem = item.find(class_=re.compile(r'price|preis', re.I))
            if price_elem:
            ...
            condition_elem = item.find(string=re.compile(r'(New|Neu|Used|Fine|Very Good|Sehr Gut|Good|Gut|Fair).*?(Hardcover|Gebunden|Softcover|Paperback|Taschenbuch)', re.I))
            if condition_elem:
                condition_text = str(condition_elem)
                binding_text = condition_text
            else:
                condition_elem = item.find(string=re.compile(r'New|Neu|Used|Fine|Very Good|Sehr Gut|Good|Gut|Fair', re.I))
                if condition_elem:
                    condition_text = str(condition_elem)

                binding_elem = item.find(string=re.compile(r'Hardcover|Gebunden|Softcover|Paperback|Taschenbuch', re.I))
                if binding_elem:
            ...
            # Extract seller
            seller = None
            seller_elem = item.find('a', href=re.compile(r'/shop/'))
            if seller_elem:
    ...
    results_text = soup.find(string=re.compile(r'(\d+)\s*(results?|Ergebnisse?)', re.I))
    if results_text:
...
def extract_ml_features(parsed_data: Dict[str, Any]) -> Dict[str, Any]:
    """Extract ML-ready features from parsed Zvab data."""
    stats = parsed_data.get("stats", {})

    features = {
        "zvab_min_price": stats.get("min_price", None),
        "zvab_avg_price": stats.get("avg_price", None),
        "zvab_seller_count": stats.get("count", 0),
    ...
    if stats.get("min_price") and stats.get("max_price"):
        features["zvab_condition_spread"] = round(
            stats["max_price"] - stats["min_price"], 2
        )

    # Has new/used
    by_condition = stats.get("by_condition", {})
    features["zvab_has_new"] = "New" in by_condition
    features["zvab_has_used"] = any(
    ...
    if "Hardcover" in by_binding and ("Softcover" in by_binding or "Paperback" in by_binding):
        ...
        if sc_price:
            features["zvab_hardcover_premium"] = round(hc_price - sc_price, 2)
