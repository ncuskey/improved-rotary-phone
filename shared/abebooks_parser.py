"""
AbeBooks HTML parser for extracting book pricing and availability data.

Parses search results from AbeBooks to extract:
...
def parse_abebooks_html(html: str, isbn: str) -> Dict[str, Any]:
    """
    Parse AbeBooks search results HTML and extract book pricing data.

    Args:
        html: Raw HTML from AbeBooks search results page
    ...
    # AbeBooks typically uses classes like "result-item", "search-result", or similar
    ...
    if not result_items:
        price_elements = soup.find_all(text=re.compile(r'US\$\s*\d+'))
...
def extract_ml_features(parsed_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Extract ML-ready features from parsed AbeBooks data.

    Args:
        parsed_data: Output from parse_abebooks_html()

    Returns:
        Dict with ML features:
        {
            "abebooks_min_price": float,
            "abebooks_avg_price": float,
            "abebooks_seller_count": int,
            "abebooks_condition_spread": float,  # Max - Min price
            "abebooks_has_new": bool,
            "abebooks_has_used": bool,
            "abebooks_hardcover_premium": float  # HC avg - SC avg (if both exist)
        }
    """
    stats = parsed_data.get("stats", {})

    features = {
        "abebooks_min_price": stats.get("min_price", None),
        "abebooks_avg_price": stats.get("avg_price", None),
        "abebooks_seller_count": stats.get("count", 0),
        "abebooks_condition_spread": None,
        "abebooks_has_new": False,
        "abebooks_has_used": False,
        "abebooks_hardcover_premium": None
    }

    # Condition spread
    if stats.get("min_price") and stats.get("max_price"):
        features["abebooks_condition_spread"] = round(
            stats["max_price"] - stats["min_price"], 2
        )

    # Has new/used
    by_condition = stats.get("by_condition", {})
    features["abebooks_has_new"] = "New" in by_condition
    features["abebooks_has_used"] = any(
    ...
    if "Hardcover" in by_binding and ("Softcover" in by_binding or "Paperback" in by_binding):
        ...
        if sc_price:
            features["abebooks_hardcover_premium"] = round(hc_price - sc_price, 2)
...
if __name__ == "__main__":
    ...
    print("Testing AbeBooks Parser")
    print("=" * 80)

    result = parse_abebooks_html(sample_html, "9780553381702")
